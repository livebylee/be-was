...
> 그 중 sendRedirct 메서드를 구현하는 과정에서, 302를 가장 많이 쓸텐데 이를 기본값으로 처리하고 싶었다.
> default parameter 로 302를 처리하고싶었지만 자바에는 그런 문법이 없는 것 같았고..
> 따라서 메서드 오버로딩을 통해서 처리하였다.
> 메서드가 두개가 생기니까 비효율적인 것 같다고 생각했는데 자바에서는 이게 더 나은 로직..? 인 것 같았다.

> ### 상태코드의 기본값?
> +) Spring 에서도 기본값으로 200 ok를 쓴다고 한다.. 웹서버의 요청 대다수는 get,post로 성공적일 것이기 때문.,